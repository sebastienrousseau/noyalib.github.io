...
- `noyalib_wasm_bg.js`, `noyalib_wasm_bg.wasm`: the wasm-pack `bundler`
  output of noyalib-wasm at tag `v0.0.35`, built locally with wasm-pack 0.15.0
  (the npm 0.0.35 tarball lacks `noyalib_wasm_bg.js`; 0.0.36 gates the package).
  Replace both files with the ones from the npm tarball at the next lockstep
  release.
